...
exports.addItemToCart = asyncHandler(async(req, res, next) => {
    ...
    if (!item) {
        return next(new ErrorResponse(`Item with id of ${req.params.itemId} not found`, 404));
    }
...
exports.removeItemFromCart = asyncHandler(async(req, res, next) => {
    ...
    item = await Item.findByIdAndUpdate(req.params.itemId, {
        incart: false
    }, {
        new: true
    })
    ...
    const removeIndex = user.cart.map(item => item._id).indexOf(req.params.itemId);
